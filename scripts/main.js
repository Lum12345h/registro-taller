document.addEventListener("DOMContentLoaded", () => {
    ...
    function getAllVehiclesFromDB() {
        return new Promise((resolve, reject) => {
            ...
            // Need a readonly transaction to get data
    ...
    async function showVehicleDetail(vehicleId) {
         ...
         if (vehiculo.fechaEntrega === null) { // Check explicitly for null
            document.getElementById('btn-mark-delivered').addEventListener('click', () => {
                 markVehicleAsDelivered(vehicleId); // Pass the unique ID
            });
         }
    ...
    async function markVehicleAsDelivered(vehicleId) {
        // Fetch the specific vehicle to update
        ...
        if (vehicleToUpdate) {
            ...
            vehicleToUpdate.fechaEntrega = getFechaActualISO(); // Set delivery date
            vehicleToUpdate.cobrado = cobrado;
            vehicleToUpdate.gastado = gastado;

            // Update the vehicle in IndexedDB
            try {
                await updateVehiculo(vehicleToUpdate); // Await the update
                alert(`Vehículo ${vehicleToUpdate.marca} ${vehicleToUpdate.modelo} marcado como entregado.`); // Feedback

                 // Go back to the pending list and re-render it (it will now exclude the delivered vehicle)
                renderVehiculosPendientes(); // This is async, but the event listener doesn't need to await it
                showScreen('pending-vehicles-screen');

                // --- Optional: Send updated delivery/cost info to Google Sheets ---
                 const dataToSendToSheet = {
                     // Include essential data to identify the row/vehicle if needed in Sheets
                     fechaIngreso: vehicleToUpdate.fechaIngreso, // Key to potentially find the row
                     placas: vehicleToUpdate.placas, // Another identifier

                     // Updated delivery/cost info
                     fechaEntrega: vehicleToUpdate.fechaEntrega,
                     cobrado: vehicleToUpdate.cobrado,
                     gastado: vehicleToUpdate.gastado,
                     // Note: Updating existing rows in Apps Script is more complex than adding.
                     // This simple script only ADDS rows.
                     // To UPDATE, you'd need to search the sheet by fechaIngreso or placas.
                     // For now, the Apps Script will just add a *new* row with the delivery info,
                     // which isn't ideal for a central register.
                     // A better approach in Apps Script would be to find the row by fechaIngreso and update it.
                     // However, sending the info is the first step if you build that later.
                      action: 'update_delivery_status' // Add an action flag for Apps Script if it handles updates
                 };

                 // If your Apps Script is set up to handle updates based on fechaIngreso/placas,
                 // uncomment this block:
                 /*
                 try {
                     const response = await fetch(GOOGLE_APPS_SCRIPT_URL, {
                          method: 'POST',
                          mode: 'no-cors', // Or 'cors' if your deployment allows
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify(dataToSendToSheet)
                     });
                     console.log("Delivery status sent to Apps Script. Response (if visible):", response);
                 } catch (sheetUpdateError) {
                     console.error("Error sending delivery update to Google Sheets:", sheetUpdateError);
                     // Alert user, but don't block the process as local save was successful
    ...
    customerDetailsForm.addEventListener("submit", async (e) => {
         ...
         try {
             ...
             // --- Paso 2: Enviar datos a Google Sheets ---
             ...
             const dataToSendToSheet = {
                 ...
                 logo: tempVehicleData.logo, // Also send logo path if needed in Sheets/Apps Script
                 ...
                 action: 'add_new_vehicle' // Add an action flag for Apps Script if it handles multiple actions
             };

             // Use a simple alert for now, but fetch below
             ...
             try {
                 ...
                 console.log("Petición a Apps Script enviada. Respuesta (si visible):", response);
...
// This function needs to be accessible here. Assuming date.js is loaded via <script> tag before main.js.
// If date.js is not loaded globally, you would need to include its content or import it.
// For this structure, keep it simple and ensure date.js is a global script loaded first.

/*
// Example content of utils/date.js
